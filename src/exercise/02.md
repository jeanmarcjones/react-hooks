...
* When passing a function to `useState` it's only called when the components first render.
* `useEffect` does a shallow comparison on items in the deps array so any non-primitives will cause it to run on a component re-render. 
